...
public class AdsRewarded : MonoBehaviour, IUnityAdsListener
{
    ...
    {   // Check if UnityAds ready before calling Show method:
        if(FindObjectOfType<HomeController>() && FindObjectOfType<HomeController>().playerData.LimitAds > 0)
        {
        ...
        else if(FindObjectOfType<Player>() && FindObjectOfType<Player>().playerData.LimitAds > 0)
        {
            if (Advertisement.IsReady(mySurfacingId)) Advertisement.Show(mySurfacingId);
            else Debug.Log("Rewarded video is not ready at the moment! Please try again later!");
        }
        ...
        {
            if(FindObjectOfType<HomeController>())
            {
                FindObjectOfType<HomeController>().HomeMsg("Hết lần xem.");
            }
            else
            {
                FindObjectOfType<GameController>().HomeMsg("Hết lần xem.");
            }
        }
    ...
    public void OnUnityAdsDidFinish(string surfacingId, ShowResult showResult) // Implement IUnityAdsListener interface methods:
    {
        if (showResult == ShowResult.Finished)
        {
            ...
            if(FindObjectOfType<HomeController>())
                FindObjectOfType<HomeController>().AdsAddCash();
            if(FindObjectOfType<GameController>())
                FindObjectOfType<GameController>().AdsAddCash();
        }
        ...
        else if (showResult == ShowResult.Failed)
        {
            ...
            if(FindObjectOfType<HomeController>())
            {
                FindObjectOfType<HomeController>().HomeMsg("Lỗi! Vui lòng thử lại.");
            }
            else
            {
                FindObjectOfType<GameController>().HomeMsg("Lỗi! Vui lòng thử lại.");
            }
